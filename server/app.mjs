...
export async function createApp({ dataDir, now = Date.now, twitchOptions = {}, allowedOrigins = [], panelDir = productionPanelDir } = {}) {
  ...
  const contest = await createContest(join(dataDir, 'contest.json'), { now });
  const twitch = await createTwitch({ filename: join(dataDir, 'twitch.json'), store, now, clientId: TWITCH_CLIENT_ID, onChatMessage: message => contest.chat(message), ...twitchOptions });
  const inputOverlay = await createInputOverlay(join(dataDir, 'input-overlay.json'));
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'GET' && path === '/api/input-overlay') return json(response, 200, inputOverlay.snapshot());
      if (request.method === 'GET' && path === '/api/contest') return json(response, 200, contest.snapshot());
      if (request.method === 'GET' && path === '/api/input-overlay/events') {
        ...
        send(inputOverlay.snapshot());
        const off = inputOverlay.subscribe(send);
        streams.add(response);
        const heartbeat = setInterval(() => response.write(': heartbeat\n\n'), 15000);
        request.on('close', () => { clearInterval(heartbeat); off(); streams.delete(response); });
        return;
      }
      if (request.method === 'GET' && path === '/api/events') {
        ...
        const sendContest = state => { if (!response.destroyed) response.write(`event: contest\ndata: ${JSON.stringify(state)}\n\n`); };
        sendContest(contest.snapshot());
        const offContest = contest.subscribe(sendContest);
        streams.add(response);
        const heartbeat = setInterval(() => response.write(': heartbeat\n\n'), 15000);
        request.on('close', () => { clearInterval(heartbeat); off(); offTwitch(); offContest(); streams.delete(response); });
        return;
      }
      if (request.method === 'POST' && path === '/api/stream-dock') {
        if (!request.headers['content-type']?.startsWith('application/json') || request.headers['x-projectchat-control'] !== 'stream-dock-v1') return json(response, 415, { error: 'Ожидается команда Cue Stream Dock.' });
        let bytes = 0, text = '';
        ...
        if (input?.action === 'start' || (input?.action === 'toggle-poll' && state.poll?.status !== 'running')) {
          ...
          if (state.draft.source === 'twitch') {
            ...
            requireThat(connection.phase === 'connected', 'Сначала подключите Twitch в Cue.', 409);
      ...
      if (request.method === 'POST' && ['/api/command', '/api/twitch', '/api/input-overlay', '/api/contest'].includes(path)) {
        ...
        if (path === '/api/contest') {
          if (command.type === 'start' && command.setup?.source !== 'test') {
            const connection = twitch.snapshot();
          ...
          return json(response, 200, await contest.command(command));
        }
        if (path === '/api/input-overlay') {
          if (command.type === 'update') return json(response, 200, await inputOverlay.update(command.config));
          if (command.type === 'demo') return json(response, 200, inputOverlay.demo());
          if (command.type === 'reset') { inputOverlay.reset(); return json(response, 200, inputOverlay.snapshot()); }
          throw new PollError('Неизвестная команда оверлея ввода.');
        }
  ...
  return {
    ...
    async listen(port = 4317) {
      ...
      ticker = setInterval(() => {
        store.command({ type: 'tick' }).catch(error => console.error('Не удалось завершить опрос:', error.message));
        contest.tick().catch(error => console.error('Не удалось завершить набор конкурса:', error.message));
      }, 250);
      void twitch.start();
      return `http://127.0.0.1:${actual}`;
    },
    async close() { clearInterval(ticker); await twitch.close(); await inputOverlay.close(); for (const stream of streams) stream.end(); await new Promise(resolve => server.close(resolve)); await store.drain(); await contest.drain(); },
  };
}
